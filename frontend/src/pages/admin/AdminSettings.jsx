...
const AdminSettings = () => {
  const [settings, setSettings] = useState({
    ai_threshold: 0.75, // Đã đổi tên biến cho khớp với API Backend (ai_threshold thay vì ai_confidence_threshold)
    max_upload_size_free: 5 // Đã đổi tên biến cho khớp với API Backend
  });
  ...
  useEffect(() => {
    const fetchSettings = async () => {
      try {
        ...
        // Cập nhật state nếu Backend trả về data hợp lệ
        if (data) {
           setSettings({
    ...
    fetchSettings();
  }, []); // Mảng rỗng: Chỉ gọi 1 lần khi load trang
        ...
        {isLoading ? (
          <div className="p-6 space-y-8">
            <div className="space-y-4">
              ...
              <div className="w-full h-11 bg-slate-700/50 rounded-lg animate-pulse"></div>
            </div>
            <div className="space-y-4">
              <div className="w-48 h-4 bg-slate-700/50 rounded animate-pulse"></div>
              <div className="w-full h-11 bg-slate-700/50 rounded-lg animate-pulse"></div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="space-y-3">
                <label htmlFor="ai_threshold" className="flex items-center gap-2 text-sm font-medium text-slate-300">
                  ...
                  Ngưỡng tự tin AI (Threshold)
                </label>
                <p className="text-xs text-slate-500">
                  Ngưỡng (0.00 - 1.00) để AI tự động đánh nhãn mà không cần duyệt.
                </p>
