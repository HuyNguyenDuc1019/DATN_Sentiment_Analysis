...
const FeedbackModal = ({ item, onClose, onSuccess }) => {
  ...
  useEffect(() => {
    if (item) {
      setSelected(item.prediction === 1 ? 0 : 1);
    }
  }, [item]);
  ...
  const handleSave = async () => {
    ...
    try {
      await submitFeedback({
        ...
        old_ai_label: item.prediction,
        corrected_label: selected,
        user_id: userId,
      });
        ...
        >
          ...
          >
            <div className="flex items-center justify-between px-6 py-5 border-b border-border dark:border-slate-700">
              <div>
                <h2 className="font-display font-bold text-ink dark:text-white text-lg">Sửa nhãn AI</h2>
            ...
            <div className="px-6 py-5 space-y-5">
              <div>
                ...
                <div className="bg-slate-50 dark:bg-slate-900/50 rounded-xl px-4 py-3 text-sm text-slate-700 dark:text-slate-300 leading-relaxed max-h-32 overflow-y-auto">
                  {item.text}
                </div>
              </div>

              <div>
                <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">Nhãn AI dự đoán</p>
                <Badge prediction={item.prediction} />
              </div>

              <div>
                <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-3">Nhãn đúng</p>
            ...
            <div className="flex gap-3 px-6 pb-5">
              ...
              <button
                type="button"
                onClick={handleSave}
                disabled={loading || selected === null || selected === item.prediction}
