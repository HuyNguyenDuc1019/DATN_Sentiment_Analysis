...
async def predict_batch(req_obj: Request, request: BatchPredictRequest):
    ...
    try:
        ...
        sys_settings = settings_res.data or {}
        ...
        dynamic_aspects = admin_aspects.copy() if isinstance(admin_aspects, dict) else {}
                ...
                if isinstance(user_aspects, dict):
                    for aspect, keywords in user_aspects.items():
                        if aspect in dynamic_aspects:
                            current_keywords = dynamic_aspects[aspect]

                            if isinstance(current_keywords, list):
                                if isinstance(keywords, list):
                                    dynamic_aspects[aspect] = current_keywords + keywords
                                else:
                                    dynamic_aspects[aspect] = current_keywords + [str(keywords)]
                            else:
                                dynamic_aspects[aspect] = f"{current_keywords}, {keywords}"
                        else:
    ...
    # Dataset metadata để Settings có thể xóa riêng từng file/quán.
    ...
    dataset_type = infer_dataset_type(request.source_url)
