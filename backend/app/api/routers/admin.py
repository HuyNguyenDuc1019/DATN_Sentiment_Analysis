...
async def get_admin_transactions(admin_id: str = Depends(verify_admin)):
    try:
        res = (
            ...
            .table("transactions")
            .select(
                "id, user_id, amount, status, plan_name, duration_days, "
                "payment_method, payment_code, paid_at, expires_at, created_at"
            )
            .order("created_at", desc=True)
            .execute()
        )
        ...
        for item in transactions:
            item["profiles"] = profiles_by_id.get(item.get("user_id"))
        ...
        print(f"Lỗi API admin transactions: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def confirm_admin_transaction(
    ...
    try:
        ...
        if transaction.get("status") == "cancelled":
            raise HTTPException(
                status_code=400,
                detail="Không thể xác nhận giao dịch đã hủy.",
            )
        ...
        if not profile_res.data:
            raise HTTPException(
                status_code=400,
                detail="Không thể nâng cấp tài khoản VIP.",
            )
        ...
        updated_transaction = (
            transaction_update_res.data[0]
            if transaction_update_res.data
            else transaction
        )
        ...
        try:
            supabase.table("admin_activity_logs").insert({
                ...
                "description": f"Admin đã xác nhận giao dịch {transaction.get('payment_code') or request.transaction_id}.",
                "created_at": now.isoformat(),
            }).execute()
        except Exception as log_error:
            print(f"⚠️ Không thể ghi admin_activity_logs: {log_error}")
        ...
        return {
            "status": "success",
            "message": "Đã xác nhận giao dịch và nâng cấp VIP cho người dùng.",
        ...
        print(f"Lỗi xác nhận giao dịch: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def cancel_admin_transaction(
    ...
    try:
        ...
        if transaction.get("status") == "paid":
            raise HTTPException(
                status_code=400,
                detail="Không thể hủy giao dịch đã thanh toán.",
            )

        if transaction.get("status") == "cancelled":
            return {
                "status": "success",
                "message": "Giao dịch này đã được hủy trước đó.",
                "transaction": transaction,
            }
        ...
        try:
            supabase.table("admin_activity_logs").insert({
                ...
                "target_type": "transaction",
                "target_id": request.transaction_id,
                "description": f"Admin đã hủy giao dịch {transaction.get('payment_code') or request.transaction_id}.",
                "created_at": now.isoformat(),
            }).execute()
        except Exception as log_error:
            print(f"⚠️ Không thể ghi admin_activity_logs: {log_error}")
