...
def verify_admin(admin_id: str):
    try:
        response = supabase.table('profiles').select('role').eq('id', admin_id).execute()
...
async def get_admin_feedbacks(admin_id: str = Depends(verify_admin)):
    try:
        ...
        profiles_dict = {p["id"]: p for p in (profiles_res.data or [])}

        reviews_res = supabase.table("scraped_reviews").select("id, user_id, content, ai_label, confidence, created_at").order("created_at", desc=True).execute()
        reviews = reviews_res.data or []

        review_lookup = {}
        for review in reviews:
            key = (review.get("user_id"), (review.get("content") or "").strip().lower(), str(review.get("ai_label")))
        ...
        for item in feedback_rows:
            ...
            key = (item.get("user_id"), (item.get("original_content") or "").strip().lower(), str(item.get("old_ai_label")))
...
async def review_feedback(request: AdminFeedbackReview, admin_id: str = Depends(verify_admin)):
    try:
...
@router.put("/settings")
async def update_system_settings(request: AdminSettingUpdate, admin_id: str = Depends(verify_admin)):
    try:
...
async def get_feedback_confidence_map(admin_id: str = Depends(verify_admin)):
    try:
        feedback_res = supabase.table('feedback_data').select('id, scraped_review_id').not_.is_('scraped_review_id', 'null').execute()
...
async def get_admin_transactions(admin_id: str = Depends(verify_admin)):
    try:
        res = supabase.table('transactions').select('id, amount, status, created_at, profiles(email, full_name)').order('created_at', desc=True).execute()
        return res.data
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@router.get("/feedback/{feedback_id}/detail")
async def get_admin_feedback_detail(feedback_id: str, admin_id: str = Depends(verify_admin)):
    try:
        res = supabase.table("feedback_data").select("*").eq("id", feedback_id).single().execute()
        if not res.data:
            raise HTTPException(status_code=404, detail="Không tìm thấy phản hồi này.")

        item = res.data
        profile_res = supabase.table("profiles").select("id, email, full_name").eq("id", item.get("user_id")).execute()
        ...
        if item.get("scraped_review_id"):
            review_res = supabase.table("scraped_reviews").select("confidence").eq("id", item.get("scraped_review_id")).execute()
            if review_res.data:
                item["ai_confidence"] = review_res.data[0].get("confidence")

        if item["ai_confidence"] is None:
            review_res = supabase.table("scraped_reviews").select("confidence, created_at").eq("user_id", item.get("user_id")).eq("content", item.get("original_content")).eq("ai_label", item.get("old_ai_label")).order("created_at", desc=True).limit(1).execute()
            if review_res.data:
                item["ai_confidence"] = review_res.data[0].get("confidence")

        if item["ai_confidence"] is None:
            review_res = supabase.table("scraped_reviews").select("confidence, created_at").eq("user_id", item.get("user_id")).eq("content", item.get("original_content")).order("created_at", desc=True).limit(1).execute()
            if review_res.data:
                item["ai_confidence"] = review_res.data[0].get("confidence")
...
async def review_feedback_detailed(request: AdminFeedbackDetailReview, admin_id: str = Depends(verify_admin)):
...
async def bulk_review_feedback(request: AdminFeedbackBulkReview, admin_id: str = Depends(verify_admin)):
    ...
    try:
        ...
        update_payload = {}
        if request.action == "approve": update_payload["status"] = "approved"
        elif request.action == "reject": update_payload["status"] = "rejected"
        elif request.action == "edit_label":
            if request.new_label is None: raise HTTPException(status_code=400, detail="Thiếu nhãn mới (new_label).")
            update_payload["corrected_label"] = request.new_label
        else: raise HTTPException(status_code=400, detail="Hành động không hợp lệ!")
...
async def export_selected_feedback(request: AdminFeedbackExportSelected, admin_id: str = Depends(verify_admin)):
    try:
        if not request.feedback_ids:
        ...
        for item in response.data:
            writer.writerow([item.get('id'), item.get('original_content'), item.get('old_ai_label'), item.get('corrected_label'), item.get('status'), item.get('user_id'), item.get('created_at')])
        output.seek(0)
        return StreamingResponse(iter([output.getvalue()]), media_type="text/csv", headers={"Content-Disposition": "attachment; filename=selected_feedback_export.csv"})
    except Exception as e:
        if isinstance(e, HTTPException): raise e
        raise HTTPException(status_code=500, detail=str(e))

@router.put("/feedback/retrain-flag")
async def update_feedback_retrain_flag(request: AdminRetrainFlagRequest, admin_id: str = Depends(verify_admin)):
    try:
        supabase.table("feedback_data").update({"include_retrain": request.include_retrain}).eq("id", request.feedback_id).execute()
        return {"status": "success", "message": "Đã cập nhật cờ retrain."}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_admin_users_activity_summary(admin_id: str = Depends(verify_admin)):
    try:
        summary = {}
        reviews_res = supabase.table("scraped_reviews").select("user_id, created_at").execute()
        for row in reviews_res.data or []:
            user_id = row.get("user_id")
            if not user_id: continue
            if user_id not in summary: summary[user_id] = {"review_count": 0, "feedback_count": 0, "last_activity_at": None}
            summary[user_id]["review_count"] += 1
            created_at = row.get("created_at")
            if created_at and (not summary[user_id]["last_activity_at"] or str(created_at) > str(summary[user_id]["last_activity_at"])):
                summary[user_id]["last_activity_at"] = created_at
        feedback_res = supabase.table("feedback_data").select("user_id, created_at").execute()
...
async def get_admin_user_activity_history(user_id: str, admin_id: str = Depends(verify_admin), limit: int = 20):
    try:
        safe_limit = max(1, min(int(limit or 20), 50))
        logs_res = supabase.table("admin_activity_logs").select("id, admin_id, admin_name, action_type, target_type, target_id, description, created_at").eq("target_id", user_id).order("created_at", desc=True).limit(safe_limit).execute()
